...
class FoodfarmLatestTweetSettings {
    ...
    public function add_plugin_page() {
        ...
        add_options_page(
                'Foodfarm Latest Tweets', 'Foodfarm Latest Tweets', 'manage_options', 'latest-tweet', array($this, 'create_admin_page')
        );
    }
    ...
    public function create_admin_page() {
        // Set class property
        $this->options = get_option('foodfarm_latest_tweet');
        ?>
        <div class="wrap">
            <?php screen_icon(); ?>          
            <form method="post" action="options.php">
                ...
                settings_fields('foodfarm_latest_tweet_group');
    ...
    public function page_init() {
        register_setting(
                'foodfarm_latest_tweet_group', // Option group
                'foodfarm_latest_tweet' // Option name
        );
    ...
    public function username_callback() {
        printf(
                '<input type="text" id="username" size="50" name="foodfarm_latest_tweet[username]" value="%s" />', isset($this->options['username']) ? esc_attr($this->options['username']) : ''
        );
    }
    
    public function consumer_key_callback() {
        printf(
                '<input type="text" size="100" name="foodfarm_latest_tweet[consumer_key]" value="%s"/><br>',
    ...
    public function consumer_secret_callback() {
        printf(
                '<input type="text" size="100" name="foodfarm_latest_tweet[consumer_secret]" value="%s"/><br>',
    ...
    public function access_token_callback() {
        printf(
                '<input type="text" size="100" name="foodfarm_latest_tweet[access_token]" value="%s"/><br>',
    ...
    public function access_token_secret_callback() {
        printf(
                '<input type="text" size="100" name="foodfarm_latest_tweet[access_token_secret]" value="%s"/><br>',
...
new FoodfarmLatestTweetSettings();