...
if( ! class_exists( 'KBE_PluginUpdateChecker' ) ) {
	...
	class KBE_PluginUpdateChecker {
		...
		function requestInfo($queryArgs = array()){
			...
			if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
				$pluginInfo = KBE_PluginInfo::fromJson($result['body']);
			}
		...
		function requestUpdate(){
			...
			return KBE_PluginUpdate::fromPluginInfo($pluginInfo);
		}
...
if ( ! class_exists( 'KBE_PluginInfo' ) ) {
...
if ( ! class_exists( 'KBE_PluginUpdate' ) ) {
	...
	class KBE_PluginUpdate {
		...
		public static function fromJson($json){
			...
			$pluginInfo = KBE_PluginInfo::fromJson($json);
			if ( $pluginInfo != null ) {
				return KBE_PluginUpdate::fromPluginInfo($pluginInfo);
			} else {
				return null;
