					...
					<div class="footer-container row">
						<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
							<div class="footer-home">
								<?php
		                        if ($foodfarm_settings['logo-footer-5'] && $foodfarm_settings['logo-footer-5']['url']):
		                            echo '<img class="" src="' . esc_url(str_replace(array('http:', 'https:'), '', $foodfarm_settings['logo-footer-5']['url'])) . '" alt="' . esc_attr(get_bloginfo('name', 'display')) . '" />';
		                        else:
		                            bloginfo('name');
		                        endif;
		                        ?>
						...
						<?php
					        $cols = 0;
					        for ($i = 1; $i <= 4; $i++) {
					            if (is_active_sidebar('footer-store-' . $i))
					                $cols++;
					        }
				        ?>
			                    ...
			                    for ($i = 1; $i <= 4; $i++) {
			                        if (is_active_sidebar('footer-store-' . $i)) {
			                            ...
			                            <div class="<?php echo esc_attr($col_class[$cols++]) ?>">
			                                <?php dynamic_sidebar('footer-store-' . $i); ?>
			                            </div>
			                            <?php
			                        }
			                    }
