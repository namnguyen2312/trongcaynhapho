...
if ( ! class_exists( 'Foodfarm_Related_Portfolios' ) ) :

class Foodfarm_Related_Knowledge {
    ...
    function __construct(){
        ...
        $this->supported_post_types = array( 'kbe_knowledgebase' );
    ...
    function related_add_metabox( $post_type ) {
        if ( in_array( $post_type, $this->supported_post_types ) ) {
            add_meta_box(
                'related-metabox',
                esc_html__( 'Related post', 'foodfarm' ),
...
function sn_related_init() {
    new Foodfarm_Related_Knowledge();
}
