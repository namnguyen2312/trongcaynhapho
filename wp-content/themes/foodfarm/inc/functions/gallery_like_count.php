...
function foodfarm_like_scripts() {
...
add_action('init', 'foodfarm_like_scripts');

add_action('wp_ajax_nopriv_foodfarm-post-like', 'foodfarm_post_like');
add_action('wp_ajax_foodfarm-post-like', 'foodfarm_post_like');

function foodfarm_post_like() {
    $nonce = $_POST['nonce'];
    if (!wp_verify_nonce($nonce, 'ajax-nonce'))
        die('Nope!');

    if (isset($_POST['foodfarm_post_like'])) {
        $like_type = 'post';
...
function foodfarm_getPostLikeLink($post_id, $like_type = 'post') {
    if ($like_type == 'comment') {
    ...
    $output = '<a href="#" class="foodfarm-post-like' . $class . '"  data-like_type="'.$like_type.'" data-post_id="' . $post_id . '" title="' . $title . '">' . $heart . $count . '</a>';
    return $output;
}
