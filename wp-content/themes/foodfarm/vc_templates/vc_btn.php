 ...
 */
$style = $icon_bg_color = $shape = $color = $size = $custom_background = $custom_text = $align = $link = $title = $button_block = $el_class = $outline_custom_color = $outline_custom_hover_background =
...
if($style!='btn-history'){
	$button_classes[]='vc_btn3 vc_general btn ';
}else{
	$button_classes[]='btn btn-default vc_general vc_btn3';
}
$button_html = $title;
...
$icon_bg_class ='';
if( $icon_bg_color != ''){
	$icon_bg_class .= ' bg-icon ';
	$icon_style .= ' style="background-color:'.$icon_bg_color.'"';
}
if ( 'true' === $add_icon ) {
	...
	if ( $icon_wrapper ) {
		$icon_html = '<i class="vc_btn3-icon"><span class="vc_btn3-icon-inner ' . esc_attr( $icon_class ) . '"'.$icon_style.'></span></i>';
	} else {
		$icon_html = '<i class="vc_btn3-icon ' . esc_attr( $icon_class ) . '"'.$icon_style.'></i>';
	}
...
$color_styles="";
if($color !=''){
	$color_styles = 'style=" background-color:'.$color.'"';
}else{
	$button_classes[] = 'vc_btn3-color-grey';
}
...
if ( $button_classes ) {
	...
	$attributes[] = 'class="' . trim( $button_classes ) .$icon_bg_class. '"';
}
...
<div class="<?php echo trim( esc_attr( $css_class ) ) ?>"><?php if ( $use_link ) {
	...
	echo '<button ' . $attributes .$color_styles. '>' . $button_html . '</button>';
} ?></div>
