...
if ( ! class_exists( 'foodfarm_reset_filter_YITH_WCAN_Reset_Navigation_Widget' ) ) {
    /**
     * YITH WooCommerce Ajax Navigation Widget
     *
     * @since 1.0.0
     */
    class foodfarm_reset_filter_YITH_WCAN_Reset_Navigation_Widget extends WP_Widget {

        function __construct() {
            $widget_ops  = array( 'classname' => 'yith-woocommerce-ajax-product-filter yith-woo-ajax-reset-navigation yith-woo-ajax-navigation woocommerce widget_layered_nav', 'description' => __( 'Reset all filters set by YITH WooCommerce Ajax Product Filter', 'foodfarm' ) );
            $control_ops = array( 'width' => 400, 'height' => 350 );
            parent::__construct( 'yith-woo-ajax-reset-navigation', __( 'YITH WooCommerce Ajax Reset Filter', 'foodfarm' ), $widget_ops, $control_ops );
        }
        ...
        function widget( $args, $instance ) {
            ...
            if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 || apply_filters( 'yith_woocommerce_reset_filters_attributes', false ) ) {
                ...
                echo '<div class="name_label widget-content">'.esc_html__('Price : ', 'foodfarm').'<span>'.'&pound;'.$min_price.' - '.'&pound;'.$max_price.'</span></div>'; 
        ...
        function form( $instance ) {
            ...
            $defaults = array(
                'title' => '',
                'label' => __( 'Reset All Filters', 'foodfarm' )
            );
            ...
            <p>
                <label>
                    <strong><?php _e( 'Title', 'foodfarm' ) ?>:</strong><br />
                    <input class="widefat" type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" />
                </label>
            </p>
            <p>
                <label>
                    <strong><?php _e( 'Button Label', 'foodfarm' ) ?>:</strong><br />
